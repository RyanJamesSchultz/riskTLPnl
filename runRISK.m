function S=runRISK(S,rand_flag)
  ...
  for i=ns:Nv
      ...
      for j=1:Ne
          ...
          % Compute the ground motion matrices (PGVc:m/s & PGVd:mm/s).
          pgv_n=GMPE(Re(In,:),M(In,:),dep(j),vs30(In,:),dGM,   -1,GMPEflag)*dSA*0.01;     % [Ng(In) Nm]
          pgv_d=GMPE(Re(Id,:),M(Id,:),dep(j),vs30(Id,:),dGM,   -1,GMPEflag)*dSA*10;       % [Ng(Id) Nm]
          
          % Compute the average spectral acceleration (PSAi:g).
          psa_i=GMPE(Re(Ii,:),M(Ii,:),dep(j),vs30(Ii,:),dGM,   -1,GMPEflag)*dSA;          % [Ng(Ii) Nm] i.e., [1 Nm]
          psa_i=PGV2PGA(psa_i)/980.665;
          for k=1:length(psa_i)
              psa_k=PGA2PSA(psa_i(k),max(M(Ii,k),Msa),Re(Ii,k),vs30(Ii,k),S.T);
          ...
          Nd1=sum(Od1.*pop(Id,:),1)/4; Nd2=sum(Od2.*pop(Id,:),1)/4;
